...
export default function Login() {
  ...
  const validationSchema = object({
    ...
    password: string().required("Password is required").min(8, "Password must contain uppercase, lowercase, number, and special character"),
  });
  ...
  const refreshToken = async () => {
    try {
      ...
      if (!response.data.access) throw new Error("No access token received");
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-center h-screen px-6 sm:px-12 md:px-24 lg:px-48">
        <motion.div
          className="flex w-full max-w-5xl shadow-lg rounded-2xl overflow-hidden relative flex-col lg:flex-row"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
